...
#define BUF_NUM 5  //originalmente era 5. testando por conta de um possivel overflow de memoria na sdisk nova com oled
...
/*                               1234567890123456 */
#ifdef _LCD_
PROGMEM const char SPLASH1[]  = "Apple ][";
PROGMEM const char SPLASH2[]  = "     v."VER""YEAR;
...
void init(char splash)
{
	...
	//lyh configura??o das portas do LCD da SDISK original 
	//#ifdef _LCD_
	...
	// configura??o da interface com o Apple
	
	set_bit(DDRC, 1);
	set_bit(DDRC, 3);
	set_bit(PORTC, 1);

	// configura??o da interrup??o de tempo
	OCR0A = 0;
	TCCR0A = 0;
	TCCR0B = 1;
...
//lyh	if(file->name[8]=='D' && file->name[9]=='S'  && file->name[10]=='K') return 1;
